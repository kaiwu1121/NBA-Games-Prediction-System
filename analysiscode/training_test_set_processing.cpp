...
/*
void NBA_pca(std::string &year,std::string &type, const bool scaleData, double var_to_retain) {
	string fileformat = ".csv";
	string inputfile = "training_set/" + type + "/" + year + "/" + year + "training_set" + fileformat; 
        string outputfile = "training_set/" + type + "/" + year + "/" + year + "training_set_pca" + fileformat; 
 	arma::mat dataset;
...
void pre_training_testing (const std::string &year, std::string &type) {
	...
	for (unsigned i = 0; i < schedule_matrix.size(); i++) {
		...
		if (type.compare("host_visitor") == 0) {
		...
		//std::cout<<host_record.size()<<" "<<visitor_record.size()<<endl;
		if ((host_record[0] == 0  &&  host_record[1] == 0) || (visitor_record[0] == 0 && visitor_record[0] == 0)) {
			//times++;
			continue;
		} else {
...
void pre_training_testing (const std::string &year, std::string &type) {
	...
	for (unsigned i = 0; i < schedule_matrix.size(); i++) {
		...
		if (type.compare("simulation") == 0) {
			host_team_file = "data_set/" + year + "/" + year + host_team + "_data_set" + fileformat;
			visitor_team_file = "data_set/" + year + "/" + year + visitor_team + "_data_set" + fileformat;
	...
	string training_normalise_file = "training_set/" + type + "/" + year + "/" + year + "training_set_normalise" + fileformat; 
...
int main (){
	...
	//string type = "overall";
	//string type = "lastN";
	std::vector<string> types = {"overall", "host_visitor", "lastN", "cluster"};
	//bool scaleData = true;
	//double var_to_retain = 1;
	//for (unsigned i = 0; i < types.size(); i++) {
	//	pre_training_testing (year, types[i]);
		//NBA_pca(year,types[i], scaleData, var_to_retain);	
	//}
